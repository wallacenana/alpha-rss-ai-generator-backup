...
if (!class_exists('Alpha_RSS_AI_Generator')) {
    ...
    const DB_VERSION = '1.6.11';
        ...
        public static string $table_generators = '';

        public static string $table_runs = '';

        public static string $table_items = '';

        public static string $table_lists = '';

        public static string $table_list_rows = '';

        public static string $table_import_logs = '';
        ...
        public static function create_tables()
        {
            ...
            $sql_generators = "CREATE TABLE " . self::$table_generators . " (
                ...
                source_content_media_enabled tinyint(1) NOT NULL DEFAULT 0,
                ...
                source_link_style varchar(20) NOT NULL DEFAULT 'button',
        ...
        public static function normalize_generation_language_value(string $value)
        {
            $value = trim($value);
            if ($value === '' || $value === 'Português do Brasil' || $value === 'Português do Brasil' || $value === 'Português do Brasil') {
                return self::get_default_generation_language();
            }
            return $value;
        }

        public static function get_generator_status_label(string $status)
        {
            $map = array(
                'active' => 'Ativo',
                'inactive' => 'Inativo',
        ...
        public static function get_schedule_type_label(string $type)
        {
            $map = array(
        ...
        public static function get_run_status_label(string $status)
        {
            $map = array(
                'success' => 'Sucesso',
                'error' => 'Erro',
        ...
        public static function keyword_list_mode_uses_source_url(string $keyword_list_mode)
        {
            return sanitize_key($keyword_list_mode) === 'url_reference';
        }
         ...
         * @param array<string, mixed>|null $generator
         */
        public static function generator_uses_source_content_media($generator)
        {
         ...
         * @param string $keyword_list_mode
         * @return string
         */
        public static function get_default_image_source_mode(string $source_type = 'rss', string $keyword_list_mode = 'keywords')
        {
            $source_type = sanitize_key($source_type);
            $keyword_list_mode = sanitize_key($keyword_list_mode);
        ...
        /**
         * @param bool|null $legacy_pexels_enabled
         */
        /**
         * @param string $source_type
         * @param string $image_source_mode
         * @param bool|null $legacy_pexels_enabled
         * @param string $keyword_list_mode
         * @return string
         */
        public static function normalize_image_source_mode(string $source_type, string $image_source_mode = '', ?bool $legacy_pexels_enabled = null, string $keyword_list_mode = 'keywords')
        {
            $source_type = sanitize_key($source_type);
            $image_source_mode = sanitize_key($image_source_mode);
            $keyword_list_mode = sanitize_key($keyword_list_mode);
         ...
         * @return bool
         */
        public static function image_source_mode_uses_source_image(string $image_source_mode)
        {
            return in_array(sanitize_key($image_source_mode), array('rss', 'rss_or_pexels', 'rss_or_dalle'), true);
        }
         ...
         * @return bool
         */
        public static function image_source_mode_uses_pexels(string $image_source_mode)
        {
            return in_array(sanitize_key($image_source_mode), array('rss_or_pexels', 'pexels'), true);
        }
         ...
         * @return bool
         */
        public static function image_source_mode_uses_dalle(string $image_source_mode)
        {
            return in_array(sanitize_key($image_source_mode), array('rss_or_dalle', 'dalle'), true);
        }
        ...
        public static function prepare_generator_record($generator)
        {
            ...
            $generator['source_content_media_enabled'] = !empty($generator['source_content_media_enabled']) ? 1 : 0;
        ...
        /**
         * @param string $header
         * @param array<int, string> $existing_headers
         * @return string
         */
        ...
        /**
         * @param array<int, string> $headers
         * @param array<int, mixed> $row_values
         * @return array<string, string>
         */
         ...
         * @param int $list_id
         * @param array<string, mixed> $filters
         * @param array<string, mixed> $source_context_filters
         * @return array<string, mixed>|null
         */
        public static function bulk_find_next_keyword_row($list_id, $filters = array(), $source_context_filters = array())
        {
            ...
            while (true) {
                ...
                /** @var array<int, object> $pending_rows */
        ...
        public static function bulk_build_manual_generator($list, $settings = array())
        {
            ...
            return array(
                ...
                'source_content_media_enabled' => !empty($settings['source_content_media_enabled']) ? 1 : 0,
        ...
        public static function normalize_generator_payload($raw)
        {
            ...
            $payload['source_content_media_enabled'] = !empty($raw['source_content_media_enabled']) ? 1 : 0;
            ...
            $payload['source_link_style'] = isset($raw['source_link_style']) ? sanitize_key(wp_unslash($raw['source_link_style'])) : 'button';
            if (!in_array($payload['source_link_style'], array('button', 'link'), true)) {
                $payload['source_link_style'] = 'button';
            }
         ...
         * @param string|null $item_guid
         * @param string|null $item_permalink
         * @return int
         */
         ...
         * @param array<int, string> $selected_tags
         * @param int $max
         * @return array<int, string>
         */
        ...
        /**
         * @param string $html
         * @param string $prepend_embed_html
         * @param string $prepend_embed_url
         * @return string
         */
        ...
        /**
         * @param string $url
         * @param string $video_selector_class
         * @param string $image_selector_class
         * @param string $link_selector_class
         * @param array<string, mixed> $source_context_filters
         * @return array<string, mixed>
         */
        ...
        {
            if (is_array($item) && !empty($item['source_context_enriched'])) {
                return $item;
            }
        ...
        {
            if (is_array($item) && !empty($item['source_context_enriched']) && (!empty($item['source_image_url']) || !empty($item['source_link_url']) || !empty($item['source_video_url']) || !empty($item['source_video_embed_html']))) {
                return $item;
            }
         ...
         * @return string
         */
        public static function build_run_stage_token_key($token)
        {
            return 'alpha_rss_ai_run_stage_' . sanitize_key((string) $token);
        }
         ...
         */
        public static function get_run_stage_data($token)
        {
            ...
            $data = get_transient(self::build_run_stage_token_key($token));
         ...
         * @return void
         */
        public static function set_run_stage_data($token, $data, $ttl = 1800)
        {
            $token = trim((string) $token);
            if ($token === '') {
                return;
            ...
            set_transient(self::build_run_stage_token_key($token), $data, max(60, intval($ttl)));
        }
         ...
         * @return void
         */
        public static function delete_run_stage_data($token)
        {
            $token = trim((string) $token);
            if ($token === '') {
                return;
            ...
            delete_transient(self::build_run_stage_token_key($token));
        }
         ...
         */
        public static function resolve_generator_item_for_run($generator, $item_guid)
        {
            $item_guid = trim((string) $item_guid);
            if ($item_guid === '') {
            ...
            $items = self::get_rss_items_for_generator(
                $generator,
                max(30, intval(isset($generator['posts_per_run']) ? $generator['posts_per_run'] : 1) * 5),
                false,
                !empty($generator['video_selector_class']) ? sanitize_text_field((string) $generator['video_selector_class']) : '',
                !empty($generator['image_selector_class']) ? sanitize_text_field((string) $generator['image_selector_class']) : '',
                !empty($generator['link_selector_class']) ? sanitize_text_field((string) $generator['link_selector_class']) : '',
         ...
         * @return array<string, mixed>
         */
        public static function prepare_generator_item_for_content_stage($generator, $item)
        {
            if (is_array($item) && !empty($item['source_content_prepared'])) {
                return $item;
            }

            $item = self::maybe_enrich_rss_item_context($generator, $item);
            $item = self::resolve_item_media_for_generation($generator, $item);
            $item['source_content_prepared'] = 1;
        ...
        public static function build_post_data($generator, $article, $item = array())
        {
            ...
            $article = is_array($article) ? $article : (array) $article;
        ...
        public static function build_pexels_query($generator, $item, $article)
        {
            ...
            $article_title = !empty($article['title']) ? (string) $article['title'] : '';
            $source_title = !empty($item['title']) ? (string) $item['title'] : '';
            $feed_title = !empty($item['feed_title']) ? (string) $item['feed_title'] : '';
            $focus_keyword = !empty($article['focus_keyword']) ? (string) $article['focus_keyword'] : '';
            ...
            if (empty($pexels_tags)) {
                ...
                if ($focus_keyword !== '') {
                    $fallback_terms[] = $focus_keyword;
                }
                if ($article_title !== '') {
                    $fallback_terms[] = $article_title;
                }
         ...
         */
        public static function apply_generator_item_media_to_post($post_id, $generator, $item, ?array $article = null)
        {
            $post_id = intval($post_id);
            $generator = is_array($generator) ? $generator : (array) $generator;
            $item = is_array($item) ? $item : (array) $item;
            ...
            if ($post_id <= 0) {
                return new WP_Error('arc_invalid_post', 'Post invalido.');
            }

            $article_title = !empty($article) && !empty($article['title']) ? (string) $article['title'] : get_the_title($post_id);
            $use_source_content_media = self::generator_uses_source_content_media($generator);
            if ($use_source_content_media && !empty($item['source_page_outline_sections']) && is_array($item['source_page_outline_sections'])) {
                ...
                $updated_content = Alpha_RSS_AI_Generator_Helper::inject_outline_section_media_into_content(
                    $current_content,
                ...
                if ($updated_content !== '' && $updated_content !== $current_content) {
            ...
            if ($use_source_image && $has_source_image) {
                $source_image_set = (bool) self::maybe_set_source_featured_image($post_id, $item, array('title' => $article_title));
                if (!$source_image_set) {
            ...
            if ($needs_fallback_image && $use_pexels) {
                ...
                $pexels_result = self::download_and_set_featured_image_from_pexels($post_id, $generator, $item, array('title' => $article_title), $is_keyword_list);
                ...
                $dalle_result = self::download_and_set_featured_image_from_dalle($post_id, $generator, $item, array('title' => $article_title), $is_keyword_list);
            ...
            if ($treat_like_rss && !empty($item['source_video_url'])) {
         ...
         * @return int|WP_Error
         */
        public static function create_post_from_generator_item($generator, $item, ?array $article = null)
        {
            $generator = is_array($generator) ? $generator : (array) $generator;
            $item = is_array($item) ? $item : (array) $item;
            if ($article !== null) {
            ...
            $source_content_prepared = !empty($item['source_content_prepared']);
            if (!$source_content_prepared) {
                $item = self::maybe_enrich_rss_item_context($generator, $item);
            ...
            if (!$use_source_page_context && !$source_content_prepared) {
                $item = self::resolve_item_media_for_generation($generator, $item);
                if ($is_keyword_list && !$is_keyword_list_url_reference) {
                    $item['source_image_url'] = '';
                }
            ...
            if (!empty($item['source_page_outline_sections']) && is_array($item['source_page_outline_sections']) && self::generator_uses_source_content_media($generator)) {
                $content_image_size = !empty($generator['content_image_size']) ? self::normalize_image_display_size((string) $generator['content_image_size']) : 'medium';
            ...
            $defer_media_pipeline = !empty($item['defer_media_pipeline']);
            if ($defer_media_pipeline) {
                return $post_id;
            }
        ...
        public static function save_generator($raw)
        {
            ...
            $data = array(
                ...
                'source_content_media_enabled' => $payload['source_content_media_enabled'],
                'source_link_phrases' => $payload['source_link_phrases'],
                'source_link_style' => $payload['source_link_style'],
        ...
        public static function duplicate_generator($id)
        {
            ...
            $duplicated = array(
                ...
                'source_content_media_enabled' => isset($generator['source_content_media_enabled']) ? $generator['source_content_media_enabled'] : 0,
                ...
                'source_link_style' => isset($generator['source_link_style']) ? $generator['source_link_style'] : 'button',
